...
FileProgress.prototype.setComplete = function(up, info) {
    ...
    if (res.url) {
        url = res.url;
        str = "<div class='link-info'><strong>Link:</strong><a href=" + res.url + " target='_blank' > " + res.url + "</a></div>" +
            "<div class=hash><strong>Hash:</strong>" + res.hash + "</div>";
    } else {
        var domain = up.getOption('domain');
        url = domain + encodeURI(res.key);
        var link = domain + res.key;
        str = "<div class='link-info'><strong>Link:</strong><a href=" + url + " target='_blank' > " + link + "</a></div>" +
            "<div class=hash><strong>Hash:</strong>" + res.hash + "</div>";
    }

    td.html(str).removeClass();
    ...
    if (isImg) {
        ...
        $(img).on('load', function() {
            ...
            function initImg(url, key, height) {
                ...
                modalBody.find('img').attr('src', '/static/img/loading.gif');
            ...
            fopLink.attr('data-key', res.key).text('预览');
